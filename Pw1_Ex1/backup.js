...
// Create helper axes to make it clearer
...
function createMarks(x, z, size, xOffset, zOffset) {
	const opacity = calculateOpacity(x, z, xOffset, zOffset);  // calculate opacity of marks (the space contains cross mark)
	const lineMaterial = new THREE.LineBasicMaterial({
		color: 0xffffff,
		transparent: true,
		opacity: opacity // set marks to just part of scene
	});
...
function createLine(startX, endX, startZ, y, endZ, material) {
	...
	]; // add position of 2 points then we can combine to a line

	const lineGeometry = new THREE.BufferGeometry().setFromPoints(points);
	const line = new THREE.Line(lineGeometry, material); // create a line
	scene.add(line);
}
...
for (let i = 0; i < 3; ++i) { // create 3 primitives for each shape
	createAllPrimitives(i * 30); // set position for each primitive along with x-axis (0, 30, 60)
}

// Create primitive and add it to scene
function addPrimitives(geometry, x, y, z) {
	const material = new THREE.MeshNormalMaterial({
		side: THREE.DoubleSide,
	}); // define material (the appearance of surfaces under lighting)
	const mesh = new THREE.Mesh(geometry, material); // define the shape and appearance of the object in the 3D world
	mesh.position.set(x, y, z);  // set position for primitive (geometry) on the scene
	scene.add(mesh);
}
...
// Adjust the renderer's size to match the display size of the canvas it's drawing on
